...
/// Representation of an electrical connector
#[derive(Debug, Clone)]
...
impl Connector {
    ...
    pub fn pins(&self) -> CSG<()> {
        ...
        for i in 0..self.pin_count {
            ...
            let pin = CSG::cylinder(self.pin_diameter / 2.0, self.pin_length)
                .translate(x_offset, 0.0, -self.pin_length);
            pins = pins.union(&pin);
        }
    ...
    /// Generate the full connector model
    pub fn assemble(&self) -> CSG<()> {
        let mut connector = self.body();
        let pins = self.pins();
        
        connector = connector.union(&pins);
